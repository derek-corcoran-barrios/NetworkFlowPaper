...
```{r LocalCorr, fig.cap="Box plot of local correlations between all three different methods of priorization"}
...
<!-- BIEN Data Sources. The BIEN data mainly comprise herbarium collections, ecological plots and surveys (53-60), and trait observations.  For details of specimen data sources see Table S2 of Maitner et al. 2017 (50). A full listing of the herbaria data used are given in the Acknowledgements section. The observations in the BIEN database are the product of contributions by 1076 different data contributors, including numerous individual herbaria, data indexers of herbarium or plot data. Of the herbaria, 550+ are listed in Index Herbarium. Additionally, BIEN 4.1 includes data from RAINBIO, TEAM, The Royal Botanical Garden of Sydney, Australia, and NeoTropTree.  Plot data within BIEN are from the CVS, NVS, SALVIAS, VEGBANK, CTFS, FIA, MADIDI, and TEAM data networks and datasets (see http://bien.nceas.ucsb.edu/bien/data-contributors/all/). --> -->
...
<!-- Extinction Risk Calculations. Standard zonation output tabulates the proportion of each conservation remaining at each step of removal. Rows of the output table that represent e.g. 30% coverage of terrestrial area were determined by scaling the solution to the available area that is not protected, agriculture or urban. Total proportion of species range conserved at an increment of conservation action was extracted for all species current and modeled future ranges. Baseline for species range size (suitability weighted range) was the full modeled range before agriculture and urban lands were removed. Extinction risk for each species was determined by applying a species area relationship to the proportion of a species range conserved at an increment of conservation action. Exponent for SAR function used here was 0.25 (default for Zonation extinction risk output). Aggregate extinction risk for each increment of natural land was the mean of individual species extinction risk calculations. --> -->